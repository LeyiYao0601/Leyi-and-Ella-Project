...
#Read This Repository In Order 1. analysis/01_data_cleaning.Rmd reads
the raw files from data/raw/, creates participant keys, recodes
documented missing-value codes, creates provisional scores, validates
ranges, and writes a local cleaned dataset. 2.
analysis/02_data_exploration.Rmd reads the cleaned dataset, checks
structure, missingness, attrition, and descriptive distributions, then
saves local figures. 3. data/processed/analysis_long.rds is generated by
the cleaning notebook. It is required for exploration but is
